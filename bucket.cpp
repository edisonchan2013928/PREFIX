...
void envelope_point_set(statistics& stat, double k, vector<int>& E_k)
{
	...
	{
		if (fabs(stat.sorted_featureVector[i][1] - k) < stat.b_s)
			E_k.push_back(i);
	}
}
...
void bound_list(statistics& stat, double k, vector<int>& E_k, vector<bound_entry>& List)
{
	...
	{
		id = E_k[i];
		pm_value = sqrt(stat.b_s*stat.b_s - (k - stat.sorted_featureVector[id][1])*(k - stat.sorted_featureVector[id][1]));
...
void init_Bucket(statistics& stat)
{
	...
	for (int u = 0; u <= 2; u++)
	{
		...
		stat.v_L[u] = new double[dim - 1]; stat.v_U[u] = new double[dim - 1]; stat.v_R_q[u] = new double[dim - 1];
...
void bucket_algorithm_row(statistics& stat, double k, int y_index, vector<double**>& stat_plane_vec)
{
	...
	for (int i = 0; i < (int)List.size(); i++)
	{
		...
		if (List[i].is_LB == true)
		{
			...
			i_l = (int)min(i_l, stat.n_x);
			stat.B_L[i_l].push_back(id);
		}
		if (List[i].is_LB == false)
		{
			i_u = (int)min(ceil((List[i].bound_value - stat.x_L) / stat.incr_x), (double)stat.n_x);
			i_u = (int)max(i_u, 0);
	...
	for (int x_index = 0; x_index < stat.n_x; x_index++)
	{
		...
		for (int u = 0; u <= 2; u++)
		{
			...
			bandwidth_square = stat.b_s*stat.b_s;
			//Code here (Compute S_I^{(u)}(q))
			stat.q[0] = stat.x_L + x_index * stat.incr_x;
			stat.q[1] = k;
			
			stat_plane_vec[u][x_index][y_index] = (1.0 - inner_product(stat.q, stat.q, dim - 1) / bandwidth_square)*stat.C_R_q[u]
			+ (2.0 / bandwidth_square) * inner_product(stat.q, stat.v_R_q[u], dim - 1) - stat.H_R_q[u] / bandwidth_square;
		}
	}
}
