...
//PREFIX_multiple
void init_PREFIX_multiple(statistics& stat)
{
	double**temp_Plane;
	...
	for (int u = 0; u < 3; u++)
	{
		...
		stat.S_D_plane_vec[u] = new double*[stat.n_x];
		stat.S_I_plane_vec[u] = new double*[stat.n_x];
		stat.S_plane_vec[u] = new double*[stat.n_x];
		for (int x_index = 0; x_index < stat.n_x; x_index++)
		{
			stat.S_D_plane_vec[u][x_index] = new double[stat.n_y];
			stat.S_I_plane_vec[u][x_index] = new double[stat.n_y];
			stat.S_plane_vec[u][x_index] = new double[stat.n_y];
		}
	}
}

void clear_PREFIX_multiple(statistics& stat)
{
	for (int u = 0; u <= 2; u++)
	{
		for (int x_index = 0; x_index < stat.n_x; x_index++)
...
void PREFIX_multiple(statistics& stat)
{
	...
	for (int t_index = 0; t_index < stat.n_t; t_index++)
	{
		...
		if (t_index > 0)
		{
			D_largest_time = cur_time - stat.b_t;
			I_largest_time = cur_time + stat.b_t;
		...
		else
		{
			...
			left = lower_bound(stat.sorted_fV_timestamp_vec.begin(),
				stat.sorted_fV_timestamp_vec.end(), cur_time - stat.b_t);
		...
		coeff_S_W0 = 1.0 - (cur_time * cur_time) / (stat.b_t*stat.b_t);
		coeff_S_W1 = 2.0 * cur_time / (stat.b_t*stat.b_t);
...
void init_check_multiple_time(statistics& stat)
{
	...
	for (int t_index = 0; t_index < stat.n_t; t_index++)
		stat.timestamp_vec[t_index] = stat.t_L + stat.incr_t*t_index;
